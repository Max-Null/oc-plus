 ...
 * @param topK       返回条数
 */
export function rrfFuse(bm25Ranks, vecRanks, topK = 5) {
    const acc = new Map();
    // BM25 权重更高：关键词精确命中优先
...
export class VectorIndex {
    ...
    async rebuild(docs) {
        ...
        for (const d of docs) {
            ...
            if (v)
                this.vectors.set(d.filePath, v);
        }
    }
    /** 语义搜索：query 向量与所有文档向量做点积（已归一化 → 余弦） */
    async search(query, topK = 5) {
        if (!this.extractor || this.vectors.size === 0)
            return [];
...
function _regQuery(valueName) {
    const out = execSync(`reg query "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Internet Settings" /v ${valueName}`, { encoding: "utf-8", windowsHide: true });
