...
export class KnowledgeEngine {
    ...
    async searchHybrid(query, topK = 5) {
        ...
        if (bm25Results.length === 0 && vecResults.length === 0)
            return [];
        const fused = rrfFuse(bm25Results.map(r => ({ filePath: r.doc.filePath })), vecResults.map(r => ({ filePath: r.filePath })), topK);
