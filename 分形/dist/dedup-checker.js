...
// 状态管理
// ============================================================
export function readDedupState() {
    try {
...
export function writeDedupState(state) {
    try {
        fs.writeFileSync(DEDUP_STATE_FILE, JSON.stringify(state, null, 2), "utf-8");
    }
    catch { /* 静默 */ }
}
// ============================================================
 ...
 * @returns 发现的疑似重复列表
 */
export async function runDedupCheck(turnCounter, forceCheck, apiConfig, debugLog, projectDir) {
 ...
 */
export function buildDedupReminder(results) {
    if (results.length === 0)
        return "";
