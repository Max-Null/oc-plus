...
当改动文件包含以下路径模式时，额外启用「2.7 内容审查」维度：

```bash
git diff --name-only HEAD 2>/dev/null | grep -E '\.md$' || true
```

| 路径模式 | 内容类型 | 启用检查子集 |
|----------|---------|-------------|
| `**/agents/*.md` | Agent 定义 | agent-prompt / agent-frontmatter |
| `**/skills/*/SKILL.md` | Skill 定义 | skill-trigger / skill-name |
| `doc/知识/*.md` `doc/设计/*.md` | 分析/设计文档 | doc-evidence / doc-timestamp |
| `**/memories/blocks/*.md` | 记忆块 | memory-level |
| `记忆管家/*.ts` `**/plugins/*.ts` | 插件代码 | memory-level（记忆路径层级） |

向用户确认识别结果（如"检测到 Java + Vue3 混合项目，含 3 个 Agent 定义文件，本次改动涉及 Java 文件 3 个、Vue 文件 2 个、Agent 定义 1 个"），然后继续。
...
### 2.7 内容审查（Markdown / Agent / Skill / 文档）—— 步骤 0 检测到内容类型时启用

**此维度审查非代码文本内容的逻辑一致性，不检查语法或格式。** 核心原则：LLM 给 LLM 写规则时，写的人和读的人有相同的认知盲区——两个人都不知道自己不知道什么。审查者必须揪出这种盲区。

#### 2.7.1 Agent 定义（`agents/*.md`）

| # | 检查项 | 严重度 | 说明 |
|---|--------|--------|------|
| A1 | **LLM 不可执行的伪规则** | 高 | 规则如果要求 LLM 判断自己的认知状态（"记忆模糊时"、"不确定时"、"如果你不知道"），LLM 无法执行——它从不觉得自己不确定。应改为输出模式触发（"输出穷举型结论前先验证"） |
| A2 | **要求 LLM 知道它不可能知道的信息** | 高 | "关于 2025 年之后发布的工具…"——LLM 不知道训练截止日期，也不知道工具的发布时间。应改为"涉及不熟悉的 API 时先查文档" |
| A3 | **permission 与 prompt 矛盾** | 中 | frontmatter 中 `permission: { websearch: deny }` 但 prompt 中说"必须先联网查证"。检查 permission 声明的每个 deny 项是否与 prompt 中的行为指令冲突 |
| A4 | **task 白名单遗漏** | 中 | prompt 中提到"调 XX subagent"，检查 `permission.task` 是否包含该 agent |

#### 2.7.2 Skill 定义（`skills/*/SKILL.md`）

| # | 检查项 | 严重度 | 说明 |
|---|--------|--------|------|
| S1 | **触发描述过宽** | 中 | description 过于泛化（如"当用户需要帮助时"），导致 skill 在不该触发时被加载。应具体到可操作的关键词或场景 |
| S2 | **触发描述过窄** | 低 | description 过于具体（如"修改 src/auth/login.ts 时"），导致 skill 在应该触发时不被加载 |
| S3 | **能力描述与实际不符** | 高 | SKILL.md 中描述的步骤引用了不存在的工具或 API。检查正文中提到的每个工具是否在项目或 OC 中确实存在 |

#### 2.7.3 分析/设计文档（`doc/知识/`、`doc/设计/`）

| # | 检查项 | 严重度 | 说明 |
|---|--------|--------|------|
| D1 | **断言无依据来源** | 高 | 文档中的穷举型/对比型/否定型结论（"OC 只有 3 个 hook"、"A 比 B 差很多"），是否标注了验证方式（联网查证 / 源码分析）和验证时间？未标注的一律标记 |
| D2 | **对比结论过期风险** | 中 | 涉及第三方系统的能力对比，是否标注了验证时间？超过 30 天的应标注"需重新验证" |
| D3 | **从局部代码推测全貌** | 高 | 是否仅从项目现有代码推测某个外部工具的完整能力（如从 memories.ts 的 3 个 hook 推断 OC 只有 3 个 hook）？必须标注"以官方文档为准" |
| D4 | **信息源优先级错误** | 中 | 引用的信息源是否符合"官方文档 > 源码分析 > 社区博客"的优先级？以社区博客作为能力判断依据的应标记 |

#### 2.7.4 记忆块（`memories/blocks/*.md`）

| # | 检查项 | 严重度 | 说明 |
|---|--------|--------|------|
| M1 | **内容层级放错** | 中 | 检查内容是否适合当前层级。例如：OC 开发相关知识应放在项目级（`.opencode/memories/blocks/`）而非全局（`~/.config/opencode/memories/blocks/`），因为其他项目不需要 |
| M2 | **元数据行格式** | 低 | 首行是否包含标准元数据：`<!-- type: knowledge --><!-- status: auto --><!-- description: 一句话摘要 -->` |
| M3 | **正文长度** | 低 | 正文是否 ≤ 15 行？超长的知识块应按需加载而非全量注入 |

### 审查输出格式

对每个发现，标注：
- **严重程度**（高：运行时必崩溃/安全漏洞/LLM 必犯的认知错误；中：潜在 bug/性能问题/内容层级不当；低：风格/可读性/格式）
- **维度**（正确性/安全性/性能/可读性/复用性/跨边界/内容-伪规则/内容-依据/内容-层级/…）
- **具体文件和行号**
- **问题描述**
- **修复建议**
...
| 2 | 高 | 内容-伪规则 | 双星.md:44 | "记忆模糊时联网"LLM无法执行 | 改为输出模式触发 |
...
- oc-plus 项目：Agent/Skill/文档对应内容类型（如 `双星`, `记忆管家`, `技能`, `doc`）
