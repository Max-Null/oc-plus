...
| 阶段 | 优先级 | 内容 |
|------|--------|------|
| F1 反馈闭环 | 短期 | Guardian 动态阈值；记忆反馈循环（pursuit/dismissal）；ACP 保护规则；事件 hook 恢复监控；优化知识注入精准度（按任务类型关键词匹配，避免双星冗余消费） |
...
- [ ] **自动化习惯执行**（分形侧）：分形注入时，auto 状态的习惯直接改为指令语气，不再通过 question 工具确认
- [ ] **工匠 LSP 深度使用**（工匠侧）：工匠 prompt 补充 goToDefinition / callHierarchy / findReferences 等 LSP 工具用法示例
